...
- 6.1 为每个 delegate（1.1–1.9）补独立 spec（mock container + 插件），不再仅靠子类间接覆盖。
  - 进度：LayerConfigModel(`config-tracks.spec`)✅ / LayerRelativeCoords(1.4 `relative-coords.spec`)✅ / LayerAnimateState(1.5 `animate-state.spec`)✅ / LayerVisibilityZoom(1.8 `visibility-zoom.spec`)✅ / LayerScaleLegend(1.6 `scale-legend.spec`)✅ / encodeStyles(3.3 `encode-styles.spec`)✅。剩 LayerPickService(1.3a)/LayerPickingManager(1.3b)/LayerMaskManager(1.7)/TextureService(1.6/1.7) 待补。
