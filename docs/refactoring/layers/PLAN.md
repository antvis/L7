...
|      | ↳ spec-first 已做     | `layer-style-fluent.spec` 25 cases 锁定两路径分流（`startInit` pending→service）、`updateStyleAttribute` isEqual diff、`label` 直接 push 不走 diff、`filter/shape` `dataSourceNeedUpdate` 开关、`animate` `options.enable` 覆盖 quirk、`style()` 别名/旧版数组兼容 + `encodeStyle` 路由、`splitValuesAndCallbackInAttribute`、链式 return-this。delegate 提取待后续刀（行为安全网已就位）。                                                                                                                                        | 零产线改动                                                                    |
...
| 1.3  | `LayerPickingManager` | `pick/boxSelect/active/setActive/select/setSelect` + `pickedFeatureID/selectedFeatureID/currentPickId/needPick` + set/get + `hooks.beforeHighlight/beforeSelect` 编排 + `setTimeout(reRender,1)` 收敛为可读调度（~150 行）。**1.3a ☑**：`pick/boxSelect/needPick` + `currentPickId/selectedFeatureID` 搬入 delegate，BaseLayer 薄转发。**1.3b ☑**：`active/setActive/select/setSelect` + hooks 编排 + setTimeout 收敛搬入 delegate；`reRender`（protected）经 ctor 注入 `rerender` 回调桥接（箭头延迟求值），零 protected 跨类访问 | `LayerPickService` 已抽出（picking 委托），本步收口**状态+编排**              |
...
| 1.9  | `LayerSourceState`    | `source/setData/setSource/getSource/sourceOption/defaultSourceConfig/layerSource/processData/dataState` + `sourceEvent/onSourceUpdate` 监听（与 `DataSourcePlugin` 边界厘清：plugin 管初始化，delegate 管状态）（~120 行）                                                                                                                                                                                                                                                                                                         | ⏳ 边界厘清：状态字段被插件/子类直访锁死 BaseLayer（见风险 8），归阶段 3 重估 |
...
- 3.1 定义 `LayerConfigModel` 收敛三轨：`rawConfig`（入参快照）/ `needUpdateConfig`（diff 暂存）/ `getLayerConfig()`（`globalConfigService` 读回）→ 统一为 `LayerConfigModel.read()` 单一读路径 + `apply(patch)` 单一写路径，`prepareBuildModel` 消费 diff。`ILayer.getLayerConfig<T>()` 签名不变。 **（☑ 已完成；新增 `core/LayerConfigModel.ts` delegate 持 `rawConfig`+`needUpdateConfig` 单一真源，`read()`/`apply(patch)`/`hasPending()` 三方法；`configService` 经 ctor 注入（BaseLayer 该字段 `protected`，注入保持零可见性变更）；BaseLayer 删 `rawConfig`/`needUpdateConfig` 字段、`getLayerConfig<T>()`/`updateLayerConfig()`/`init()`/`prepareBuildModel()` 改薄转发；`ILayer`/公开签名零变更。先补 `config-tracks.spec.ts` 10 cases（commit 7689e78）锁定三轨可观测契约，后接 delegate——数字节级镜像、运行时零行为变化。）**
- 3.2 给 `defaultSourceConfig/sourceOption/shapeOption/encodeStyleAttribute` 补严格类型（替 `any`）：引入 `ILayerSourceOption/IShapeOption/IEncodedStyleMap`，`get(name)`/`getScale(name)` 标泛型或返回联合类型。 **（☑ 完成；`defaultSourceConfig`/`sourceOption`/`shapeOption`/`get(name)` 历史已各自定型为 `IDefaultSourceConfig`/`ISourceOption`/`IShapeOption`/`number`；`encodeStyleAttribute` → `IEncodedStyleMap`（core 新增 `IEncodedStyleValue`+`IEncodedStyleMap`）；`getScale(name):any` → `getScale<T = IStyleScale>(name): T`（core `ILayer` 接口 + layers `BaseLayer`/`LayerScaleLegend` 同步；默认 `IStyleScale` 语义收紧、运行时零变化、`getLayerAttributeScale` 底层 `any` 归后续 scale service 专属刀）。）**
...
- 6.1 为每个 delegate（1.1–1.9）补独立 spec（mock container + 插件），不再仅靠子类间接覆盖。
  - 进度：LayerConfigModel(`config-tracks.spec`)✅ / LayerRelativeCoords(1.4 `relative-coords.spec`)✅ / LayerAnimateState(1.5 `animate-state.spec`)✅ / LayerVisibilityZoom(1.8 `visibility-zoom.spec`)✅ / LayerScaleLegend(1.6 `scale-legend.spec`)✅ / LayerMaskManager(1.7 `mask-manager.spec`)✅ / LayerPickingManager(1.3b `picking-manager.spec`)✅ / LayerPickService(1.3a `pick-service.spec`)✅ / encodeStyles(3.3 `encode-styles.spec`)✅ / TextureService(1.6/1.7 `texture-service.spec`)✅。6.1 全 9/9 delegate 独立 spec 闭环。
- 6.2 ✅ 交互编码开关（`enableShaderEncodeStyles/enableDataEncodeStyles`）运行时分流 spec（`encode-switch.spec` 24 cases）：锁定 `BaseModel.getInject()`→`getDynamicStyleInject` 的 uniform/attribute 双模（按 `encodeStyleAttribute[key]` 存在与否分流）、`getStyleAttribute()` 的 `u_<key>` 默认值解析（`DefaultUniformStyleValue`/layerConfig 覆盖 + stroke→rgb2arr、anchor→anchorToNumber 变换）、`registerStyleAttribute()` 的 data 通道注册 dispatch（经 `getCommonStyleAttributeOptions` 注册表，未知 key 跳过），及 `getCommonStyleAttributeOptions` 注册表覆盖面（rotation/stroke/opacity/offsets/anchor）。3.3 已锁数据结构收敛；本 spec 补运行时分流。
...
- 6.4 ✅ `BaseLayer` 的 delegate 转发用「接口契约 spec」模式（`delegate-forward-contract.spec` 26 cases，仿 maps `base-map-event.spec.ts`）锁定对外透明：实例化后替换 `maskManager`/`pickingManager`/`visibilityZoomManager`/`scaleLegendManager`/`animateState`/`relativeCoordsManager`/`configModel` 七个 public delegate 字段为 spy stub，调每个对外 `ILayer` 公共方法断言对应 delegate 方法被以参数 verbatim 调用一次、返回值原样透传、无跨 delegate 副作用。`layerPickService`/`textureService` 为 init 期创建且转发点在 init/destroy 内部单行（非对外 `ILayer` 表面），不属本契约范围（各自 6.1 已独立覆盖）。阶段 6（6.1–6.4）全闭环。
...
8. **1.9 边界厘清（暂缓）**：`dataState`/`layerSource`/`sourceOption`/`defaultSourceConfig` 被 `DataSourcePlugin`（`layer.dataState.*`/`layer.sourceOption`/`layer.defaultSourceConfig`）、`DataMappingPlugin`（`layer.dataState.dataMappingNeedUpdate`）、子类 `raster`（`this.layerSource.getParserType()`）直访，锁死在 BaseLayer，无法搬入 delegate 而不破坏插件/子类契约。`setData/setSource/sourceEvent/onSourceUpdate/processData` 又深度依赖这些 protected 字段 + `inited/tileLayer/clusterZoom/mapService/coordCenter/reRender/fitBounds/processRelativeCoordinates` 等成员，强行抽需暴露 internal `setLayerSource` setter 并引入 `getSource`↔delegate 循环，delegate 退化为方法容器无净收益。当前 `ILayer` 接口下无零风险抽取路径 → **归阶段 3.2（接口收敛，插件改走 ILayer getter/方法而非直访字段）后重估**。阶段 1 不强行下刀。
