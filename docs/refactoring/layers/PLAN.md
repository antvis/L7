...
| 1.3  | `LayerPickingManager` | `pick/boxSelect/active/setActive/select/setSelect` + `pickedFeatureID/selectedFeatureID/currentPickId/needPick` + set/get + `hooks.beforeHighlight/beforeSelect` 编排 + `setTimeout(reRender,1)` 收敛为可读调度（~150 行）。**1.3a ☑**：`pick/boxSelect/needPick` + `currentPickId/selectedFeatureID` 搬入 delegate，BaseLayer 薄转发（零 protected 跨类、零 API 变更）。**1.3b ⏳**：`active/setActive/select/setSelect` + hooks 编排 + setTimeout 收敛（需 `reRender` protected 桥接） | `LayerPickService` 已抽出（picking 委托），本步收口**状态+编排** |
