...
**阶段 3.1.3**：`GeoJSONVTLoader` 抽取（阶段 3.1 Parser/Loader 解耦第 3 增量，阶段 3.1 收尾）。把 `parser/geojsonvt.ts` 的 `getVectorTile` 闭包（含 `geojson-vt` 内存切瓦片 + `getCustomData` 分支 + 缓存 key 逻辑）抽取为实现 `TileLoader` 接口的 `GeoJSONVTLoader` 类。关键注意：① geojsonvt 是**内存切瓦片**（非网络 fetch），`getVectorTile` 走 `geojsonvt.tile(tile.x, tile.y, tile.z)` 同步切瓦片 + `getCustomData` 分支回调取数，无 `tile.xhrCancel` 取消语义（与 mvt 不同，与 jsonTile 等价——不设 xhrCancel）；② 始终 resolve `ITileSource`（空数据 resolve 空 defaultLayer 的 `GeoJSONVTTileSource`，与 jsonTile 同形态），`undefined` 是合法但不使用的值；③ URL 插值用 `tile.x/y/z`（与 jsonTile 同、与 mvt 不同）；④ 沿用 `jest.mock('@antv/l7-utils')` + mock `GeoJSONVTTileSource` 文件级 mock 模式补 loader 单测（geojsonvt 成功/空 + getCustomData 成功/失败 + 切瓦片入参用 tile.x/y/z + 不设 xhrCancel）。⚠️geojsonvt parser 较大（~160 行，含 cluster/extent 联动），抽取时需仔细分辨哪些是「数据获取」下沉 loader、哪些是「parser 形状组装」留在 parser。详见 [PLAN.md § 阶段 3.1](./PLAN.md)。
---
