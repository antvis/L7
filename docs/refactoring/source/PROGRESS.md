...
**阶段 3.2.2**（可选 / 边际收益）：把 3.2.1 抽出的 `RasterTileLoader` 分发器 6 分支 switch 拆成 4 个独立小 loader + 引入 `RasterTileLoader` 接口。要点：① 4 个小 loader 按取数路径分组 —— `RasterImageLoader`（IMAGE+default）、`RasterBufferLoader`（ARRAYBUFFER）、`RasterCustomImageLoader`（CUSTOMIMAGE+CUSTOMTERRAINRGB）、`RasterCustomDataLoader`（CUSTOMARRAYBUFFER+CUSTOMRGB），各 `loadTile(tileParams, tile)` 显式声明参数需求；② 引入**独立** `RasterTileLoader` 接口（`loadTile` 返回影像/缓冲/栅格，**不**与矢量 `TileLoader` 合并，失败 reject 保留）；③ 分发器按 `dataType` 选 loader（`switch → new XLoader(...)`）；④ 测试沿用 3.2.1 模块 mock 模式。⚠️ **收益评估边际**：3.2.1 后 switch 已在 loader、取数 utils 已模块化，4 小 loader 各仅 2-3 行委托 —— 拆分主要是「interface 化 + 分文件」的整洁收益，无行为变化。**建议**：若用户更看重推进度，可直接跳到 3.4 `CustomDataProvider`（`getCustomData`/`getCustomImageData` 统一到 loader 接口，跨 mvt/raster 共用，价值更高）；3.2.2 可作为收尾补丁。详见 [PLAN.md § 阶段 3.2/3.4](./PLAN.md)。
---
...
- **改了什么**：
  ...
  - 新增 `__tests__/loader/image-loader.spec.ts`（117 行 / 7 tests）：文件级 `jest.mock('@antv/l7-utils')`（`getImage`/`isImageBitmap`）。**坑修复**：① `jest.resetAllMocks()`（非 `clearAllMocks`）—— 清 `mockReturnValue` 避免跨 case 泄露（case 2 `isImageBitmap.mockReturnValue(true)` 泄露到 case 3 让 string 输入误走 isImageBitmap 分支跳过 getImage，初版 5/7 失败）；② 刷微任务用 `setTimeout(0)`（jsdom 无 `setImmediate`，初版 `ReferenceError: setImmediate is not defined`）。7 case：① HTMLImageElement 直传 → resolve [data]，不调 getImage/isImageBitmap（锁短路顺序）；② ImageBitmap 直传（isImageBitmap=true）；③ 单 string url 成功 + requestParameters 透传；④ **单 string url 失败 → Promise 永不 resolve**（既存 latent bug 锁定，flag + 刷微任务断言 resolved=false）；⑤ 多 string[] 全部成功；⑥ 多 string[] 部分失败 → resolve 只含成功 img（计数到 imageCount 即 done）；⑦ 多 url 全失败 → resolve []（与单 url 失败「永不 resolve」不对称 —— 既存行为保留）。
- **设计取舍**：
