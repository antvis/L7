...
**阶段 3.2.2**：把 3.2.1 抽出的 `RasterTileLoader` 分发器中的 6 分支 switch 拆成**独立小 loader**，并引入 `RasterTileLoader` 接口（`loadTile(tileParams, tile): Promise<HTMLImageElement | ImageBitmap | raster>`）由分发器按 `dataType` 选 loader。要点：① 小 loader 按取数路径分组 —— `RasterImageLoader`（IMAGE+default → `getTileImage`）、`RasterBufferLoader`（ARRAYBUFFER → `getTileBuffer`）、`RasterCustomImageLoader`（CUSTOMIMAGE+CUSTOMTERRAINRGB → `getCustomImageData`）、`RasterCustomDataLoader`（CUSTOMARRAYBUFFER+CUSTOMRGB → `getCustomData`），共 4 个小 loader（6 分支因 CUSTOM* 两两共用取数函数，天然 4 路）；② `RasterTileLoader` 保留 `data`/`tileDataType`/`cfg`，内部 switch 改为按 `tileDataType` 选 `RasterTileLoader` 实现（`record<RasterTileType, RasterTileLoader>` 或 `switch → new XLoader(...)`），对外 `loadTile` 签名不变；③ **接口返回类型**：raster 返回影像/缓冲/栅格数据，与矢量 `TileLoader` 的 `ITileSource | undefined` 不同，故 `RasterTileLoader` 接口**独立定义**（不与矢量共用），失败语义保留 reject（消费层 `SourceTile.loadData` 的 try/catch 令 reject 与 resolve 空值都走 `onError`，故等价但机械保留）；④ **第 4 种 tile/tileParams 混用形态保留**：IMAGE/ARRAYBUFFER 双用 tileParams+tile、CUSTOM* 只用 tile，小 loader 各自显式声明其参数需求；⑤ 测试沿用 3.2.1 的 `jest.mock('../../src/utils/tile/getRasterTile'|'getCustomData')` 模块 mock 模式，每个小 loader 独立 spec（或合并在 raster-tile-loader.spec 内 describe 分组），断言取数 utils 调用入参 + 返回透传；⑥ 取数 utils（`getRasterTile.ts` / `getCustomData.ts`）暂留 `utils/tile/` 不迁，3.2.2 仅在 loader 层包一层。⚠️ 若 3.2.2 拆分收益边际（switch 已在 loader、utils 已模块化），可评估**合并到 3.2 收尾**或推迟，优先推进 3.3 `image.ts` parser 去 fetch（更高价值 —— image.ts 仍自取数）。详见 [PLAN.md § 阶段 3.2](./PLAN.md)。
---
