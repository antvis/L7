...
const inferredTypesChanged = (inferenceMetadata, prevInferenceMetadata) => Object.keys(inferenceMetadata).filter(type => inferenceMetadata[type].dirty && !haveEqualFields(inferenceMetadata[type], prevInferenceMetadata[type])).length > 0;
...
const maybeRebuildSchema = debounce(async () => {
  ...
  if (!inferredTypesChanged(inferenceMetadata, lastMetadata) && !schemaChanged(schemaCustomization, lastSchemaCustomization)) {
    return;
  }
