...
const addInferredTypes = ({
  ...
  const typesWithNodes = Object.keys(inferenceMetadata).filter(typeName => hasNodes(inferenceMetadata[typeName]));
...
const addInferredType = ({
  ...
  if (typeComposer.getExtension(`createdFrom`) === `inference` && hasNodes(inferenceMetadata[typeName])) {
    const nodes = nodeStore.getNodesByType(typeName);
    typeComposer.setExtension(`plugin`, nodes[0].internal.owner);
  }

  const exampleValue = getExampleObject(Object.assign({}, inferenceMetadata[typeName], {
    typeName,
    typeConflictReporter
  }));
  addInferredFields({
