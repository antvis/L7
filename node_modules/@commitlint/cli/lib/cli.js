...
function main(options) {
	return new Promise(function ($return, $error) {
		var raw, flags, fromStdin, range, input, messages, loadOpts, loaded, parserOpts, opts, format, results, originalInput, report, output;
		raw = options.input;
			...
			try {
				...
				return Promise.resolve(load(getSeed(flags), loadOpts)).then(function ($await_2) {
					try {
						...
						return Promise.resolve(Promise.all(messages.map(message => lint(message, loaded.rules, opts)))).then(function ($await_3) {
							try {
								results = $await_3;
