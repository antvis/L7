...
function dragWithMove(
  ...
  document.dispatchEvent(
    new MouseEvent('mousemove', { bubbles: true, buttons: 1, clientX: end.x, clientY: end.y }),
  );
  target.dispatchEvent(
    new MouseEvent('mouseup', { bubbles: true, clientX: end.x, clientY: end.y }),
  );
}
...
function touchFunctionFactory(event: string) {
  return (target: HTMLElement | Window, options?) => {
    const defaultTouches =
      event.endsWith('end') || event.endsWith('cancel') ? [] : [{ clientX: 0, clientY: 0 }];
