...
describe('#calculateCameraOptionsFromTo', () => {
  ...
  test('look at north', () => {
    const cameraOptions: CameraOptions = camera.calculateCameraOptionsFromTo(
      { lng: 1, lat: 0 },
      0,
      { lng: 1, lat: 1 },
    );
  ...
  test('look at west', () => {
    const cameraOptions = camera.calculateCameraOptionsFromTo({ lng: 1, lat: 0 }, 0, {
      lng: 0,
      lat: 0,
    });
    expect(cameraOptions).toBeDefined();
  ...
  test('zoom distance 1000', () => {
    const expectedZoom = Math.log2(
      camera.transform.cameraToCenterDistance /
        mercatorZfromAltitude(1000, 0) /
        camera.transform.tileSize,
    );
    const cameraOptions = camera.calculateCameraOptionsFromTo(
      { lng: 0, lat: 0 },
      0,
      { lng: 0, lat: 0 },
      1000,
  ...
  test('zoom distance 1 lng (111.2km), 111.2km altitude away', () => {
    const expectedZoom = Math.log2(
      camera.transform.cameraToCenterDistance /
        mercatorZfromAltitude(Math.hypot(111200, 111200), 0) /
        camera.transform.tileSize,
    );
    const cameraOptions = camera.calculateCameraOptionsFromTo(
      { lng: 0, lat: 0 },
  ...
  test('same To as From error', () => {
    expect(() => {
      camera.calculateCameraOptionsFromTo({ lng: 0, lat: 0 }, 0, { lng: 0, lat: 0 }, 0);
    }).toThrow();
  });
});
...
describe('#zoomTo', () => {
  ...
  test('zooms around specified location', () => {
    ...
    expect(fixedLngLat(camera.getCenter())).toEqual(
      fixedLngLat({ lng: 4.455905897939886, lat: 0 }),
    );
  });
  ...
  test('zooms with specified offset', () => {
    ...
    expect(camera.getZoom()).toBe(3.2);
    expect(fixedLngLat(camera.getCenter())).toEqual(
      fixedLngLat({ lng: 62.66117668978015, lat: 0 }),
    );
  });
  ...
  test('zooms with specified offset relative to viewport on a rotated camera', () => {
    ...
    expect(camera.getZoom()).toBe(3.2);
    expect(fixedLngLat(camera.getCenter())).toEqual(
      fixedLngLat({ lng: -62.66117668978012, lat: 0 }),
    );
  });
...
describe('#rotateTo', () => {
  ...
  test('rotates with specified offset', () => {
    ...
    expect(camera.getBearing()).toBe(90);
    expect(fixedLngLat(camera.getCenter())).toEqual(
      fixedLngLat({ lng: 70.3125, lat: 57.3265212252 }),
    );
  });
  ...
  test('rotates with specified offset, constrained to fit the view', () => {
    ...
    expect(camera.getBearing()).toBe(90);
    expect(fixedLngLat(camera.getCenter())).toEqual(
      fixedLngLat({ lng: 70.3125, lat: 0.000002552471840999715 }),
    );
  });
...
describe('#flyTo', () => {
  ...
  test('does not throw when cameras current zoom is above maxzoom and an offset creates infinite zoom out factor', () => {
    ...
    transform.resize(512, 512);
    const camera = attachSimulateFrame(new CameraMock(transform, {} as any)).jumpTo({
      zoom: 21,
      center: [0, 0],
    });
    camera._update = () => {};
  ...
  test('check elevation events freezeElevation=false', async () => {
    ...
    camera._prepareElevation = () => {
      terrainCallbacks.prepare++;
    };
    camera._updateElevation = () => {
      terrainCallbacks.update++;
    };
    camera._finalizeElevation = () => {
      terrainCallbacks.finalize++;
    };
    camera.setCenter([-10, 0]);
