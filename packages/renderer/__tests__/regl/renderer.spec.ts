...
describe('ReglRendererService', () => {
  ...
  it('should create model with createModel API', () => {
    const { clear, createModel, createAttribute, createBuffer } = rendererService;
  ...
  it('should draw with createElements', () => {
    const { clear, createModel, createAttribute, createBuffer, createElements } = rendererService;
    const model = createModel({
      vs: quad,
      fs: 'void main() {gl_FragColor = vec4(1., 0., 0., 1.);}',
      attributes: {
  ...
  it('should render to framebuffer with `useFramebuffer` correctly', () => {
    const { clear, createModel, createAttribute, createBuffer, createFramebuffer, useFramebuffer } =
      rendererService;
    const model = createModel({
      vs: quad,
      fs: 'void main() {gl_FragColor = vec4(1., 0., 0., 1.);}',
