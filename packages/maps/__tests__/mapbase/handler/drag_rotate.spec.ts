import { Map } from '../../../src/mapbase/map/map';
import { browser } from '../../../src/mapbase/map/util/browser';
import { DOM } from '../../../src/mapbase/map/util/dom';
import { extend } from '../../../src/mapbase/map/util/util';
...
describe('drag rotate', () => {
  ...
  test('DragRotateHandler stops firing events after mouseup', () => {
    ...
    spy.mockReset();
    simulate.mousemove(map.getCanvasContainer(), { buttons: 0, clientX: 20, clientY: 20 });
    map._renderTaskQueue.run();
  ...
  test('DragRotateHandler fires rotatestart, rotate, and rotateend events at appropriate times in response to a control-left-click drag', () => {
    ...
    simulate.mousedown(map.getCanvasContainer(), { buttons: 1, button: 0, ctrlKey: true });
    map._renderTaskQueue.run();
    expect(rotatestart).toHaveBeenCalledTimes(0);
    expect(rotate).toHaveBeenCalledTimes(0);
    expect(rotateend).toHaveBeenCalledTimes(0);
    ...
    simulate.mouseup(map.getCanvasContainer(), { buttons: 0, button: 0, ctrlKey: true });
    map._renderTaskQueue.run();
    expect(rotatestart).toHaveBeenCalledTimes(1);
    expect(rotate).toHaveBeenCalledTimes(1);
    expect(rotateend).toHaveBeenCalledTimes(1);
  ...
  test("DragRotateHandler doesn't fire rotate event when pitching only", () => {
    ...
    simulate.mousemove(map.getCanvasContainer(), { buttons: 2, clientX: 0, clientY: -10 });
    map._renderTaskQueue.run();
    expect(rotatestart).toHaveBeenCalledTimes(0);
    expect(rotate).toHaveBeenCalledTimes(0);
    expect(pitch).toHaveBeenCalledTimes(1);
  ...
  test('DragRotateHandler ends a control-left-click drag on mouseup even when the control key was previously released (#1888)', () => {
    ...
    simulate.mouseup(map.getCanvasContainer(), { buttons: 0, button: 0, ctrlKey: false });
    map._renderTaskQueue.run();
    expect(rotateend).toHaveBeenCalledTimes(1);
  ...
  test('DragRotateHandler requests a new render frame after each mousemove event', () => {
    ...
    simulate.mousemove(map.getCanvasContainer(), { buttons: 2, clientX: 20, clientY: 20 });
  ...
  test('DragRotateHandler does not end a right-button drag on left-button mouseup', () => {
    ...
    simulate.mousedown(map.getCanvasContainer(), { buttons: 3, button: 0 });
    map._renderTaskQueue.run();
    expect(rotatestart).toHaveBeenCalledTimes(1);
    expect(rotate).toHaveBeenCalledTimes(1);
    expect(rotateend).toHaveBeenCalledTimes(0);

    simulate.mouseup(map.getCanvasContainer(), { buttons: 2, button: 0 });
    map._renderTaskQueue.run();
    expect(rotatestart).toHaveBeenCalledTimes(1);
    expect(rotate).toHaveBeenCalledTimes(1);
    expect(rotateend).toHaveBeenCalledTimes(0);
  ...
  test('DragRotateHandler does not end a control-left-button drag on right-button mouseup', () => {
    ...
    simulate.mousedown(map.getCanvasContainer(), { buttons: 3, button: 2, ctrlKey: true });
    map._renderTaskQueue.run();
    expect(rotatestart).toHaveBeenCalledTimes(1);
    expect(rotate).toHaveBeenCalledTimes(1);
    expect(rotateend).toHaveBeenCalledTimes(0);

    simulate.mouseup(map.getCanvasContainer(), { buttons: 1, button: 2, ctrlKey: true });
    map._renderTaskQueue.run();
    expect(rotatestart).toHaveBeenCalledTimes(1);
    expect(rotate).toHaveBeenCalledTimes(1);
    expect(rotateend).toHaveBeenCalledTimes(0);
  ...
  test('DragRotateHandler does not begin rotation on spurious mousemove events', () => {
    ...
    simulate.mouseup(map.getCanvasContainer(), { buttons: 0, button: 2, clientX: 10, clientY: 10 });
    map._renderTaskQueue.run();
    expect(rotatestart).toHaveBeenCalledTimes(0);
    expect(rotate).toHaveBeenCalledTimes(0);
    expect(rotateend).toHaveBeenCalledTimes(0);
  ...
  test('DragRotateHandler does not begin a mouse drag if moved less than click tolerance', () => {
    ...
    simulate.mousemove(map.getCanvasContainer(), { buttons: 2, clientX: 13, clientY: 10 });
    map._renderTaskQueue.run();
    expect(rotatestart).toHaveBeenCalledTimes(0);
    expect(rotate).toHaveBeenCalledTimes(0);
    expect(rotateend).toHaveBeenCalledTimes(0);
    ...
    simulate.mousemove(map.getCanvasContainer(), { buttons: 2, clientX: 10, clientY: 13 });
    map._renderTaskQueue.run();
    expect(rotatestart).toHaveBeenCalledTimes(0);
    expect(rotate).toHaveBeenCalledTimes(0);
    expect(rotateend).toHaveBeenCalledTimes(0);
    ...
    simulate.mousemove(map.getCanvasContainer(), { buttons: 2, clientX: 14, clientY: 10 - 4 });
    map._renderTaskQueue.run();
    expect(rotatestart).toHaveBeenCalledTimes(1);
    expect(rotate).toHaveBeenCalledTimes(1);
    expect(rotateend).toHaveBeenCalledTimes(0);
