...
} from '../../../src/mapbase/map/handler/one_finger_touch_drag';
...
describe('one touch drag handler tests', () => {
  test('OneFingerTouchRotateHandler', () => {
    ...
    oneTouchRotate.dragStart(
      new TouchEvent('touchstart', { targetTouches: [testTouch] }),
      new Point(0, 0),
    );
    expect(oneTouchRotate.isActive()).toBe(false);
    ...
    expect(oneTouchRotate.dragMove(overToleranceMove, new Point(10, 10))).toEqual({
      bearingDelta: 8,
    });
  ...
  test('OneFingerTouchPitchHandler', () => {
    ...
    const overToleranceMove = new TouchEvent('touchmove', { targetTouches: [testTouch] });
    expect(oneTouchPitch.dragMove(overToleranceMove, new Point(10, 10))).toEqual({
      pitchDelta: -5,
    });
