import type { MapOptions } from '../../../src/mapbase/map/map';
...
describe('drag_pan', () => {
  ...
  test('DragPanHandler fires dragstart, drag, and dragend events at appropriate times in response to a touch-triggered drag', () => {
    ...
    simulate.touchstart(map.getCanvasContainer(), {
      touches: [{ target, clientX: 0, clientY: 0 }],
    });
    map._renderTaskQueue.run();
    expect(dragstart).toHaveBeenCalledTimes(0);
    expect(drag).toHaveBeenCalledTimes(0);
    ...
    simulate.touchmove(map.getCanvasContainer(), {
      touches: [{ target, clientX: 10, clientY: 10 }],
    });
    map._renderTaskQueue.run();
    expect(dragstart).toHaveBeenCalledTimes(1);
    expect(drag).toHaveBeenCalledTimes(1);
  ...
  test('DragPanHandler requests a new render frame after each mousemove event', () => {
    ...
    simulate.mousemove(map.getCanvasContainer(), { buttons, clientX: 20, clientY: 20 });
  ...
  ['ctrl', 'shift'].forEach((modifier) => {
    test(`DragPanHandler does not begin a drag if the ${modifier} key is down on mousedown`, () => {
      ...
      simulate.mousedown(map.getCanvasContainer(), { buttons, [`${modifier}Key`]: true });
      map._renderTaskQueue.run();
      expect(dragstart).toHaveBeenCalledTimes(0);
      expect(drag).toHaveBeenCalledTimes(0);
      expect(dragend).toHaveBeenCalledTimes(0);
      ...
      simulate.mouseup(map.getCanvasContainer(), { [`${modifier}Key`]: true });
      map._renderTaskQueue.run();
      expect(dragstart).toHaveBeenCalledTimes(0);
      expect(drag).toHaveBeenCalledTimes(0);
      expect(dragend).toHaveBeenCalledTimes(0);
  ...
  test('DragPanHandler does not end a drag on right button mouseup', () => {
    ...
    simulate.mousedown(map.getCanvasContainer(), { buttons: buttons + 2, button: 2 });
    map._renderTaskQueue.run();
    expect(dragstart).toHaveBeenCalledTimes(1);
    expect(drag).toHaveBeenCalledTimes(1);
    expect(dragend).toHaveBeenCalledTimes(0);

    simulate.mouseup(map.getCanvasContainer(), { buttons, button: 2 });
    map._renderTaskQueue.run();
    expect(dragstart).toHaveBeenCalledTimes(1);
    expect(drag).toHaveBeenCalledTimes(1);
    expect(dragend).toHaveBeenCalledTimes(0);
