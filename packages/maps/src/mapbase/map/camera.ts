...
export abstract class Camera extends Evented {
  ...
  _cameraForBoxAndBearing(
    ...
    const scaleX =
      (tr.width -
        (edgePadding.left + edgePadding.right + options.padding.left + options.padding.right)) /
      size.x;
    const scaleY =
      (tr.height -
        (edgePadding.top + edgePadding.bottom + options.padding.top + options.padding.bottom)) /
      size.y;
  ...
  calculateCameraOptionsFromTo(
    ...
    const zoom = this.transform.scaleZoom(
      this.transform.cameraToCenterDistance / distance3D / this.transform.tileSize,
    );
  ...
  easeTo(
    ...
    const tr = this._getTransformForUpdate(),
      ...
      bearing =
        'bearing' in options ? this._normalizeBearing(options.bearing, startBearing) : startBearing,
