...
export class HandlerManager {
  ...
  _addDefaultHandlers(options: CompleteMapOptions) {
    ...
    const cooperativeGestures = (map.cooperativeGestures = new CooperativeGesturesHandler(
      map,
    ...
    map.touchZoomRotate = new TwoFingersTouchZoomRotateHandler(
      el,
      touchZoom,
      touchRotate,
      tapDragZoom,
    ...
    const scrollZoom = (map.scrollZoom = new ScrollZoomHandler(map, () =>
  ...
  handleEvent = (e: Event, eventName?: keyof Handler) => {
    ...
    const points = mapTouches
      ? DOM.touchPos(this._map.getCanvasContainer(), mapTouches)
      : DOM.mousePos(this._map.getCanvasContainer(), e as MouseEvent);
  ...
  _updateMapTransform(
    ...
    const { panDelta, zoomDelta, bearingDelta, pitchDelta, pinchAround } = combinedResult;
