    ...
    async (files: Array<{ path: string; content: string }>) => {
      const newFiles = await Promise.all(
        files.map(async (file) => {
          ...
          if (
            (prefix === 'en' || prefix === 'zh') &&
            pathItems[1] !== 'examples'
          ) {
            pathstr =
              '../../docs/' +
              file.path.substring(3).replace(/\/index\.html/, `.${prefix}.md`);
          }

          if (pathItems[0] === 'api' || pathItems[0] === 'tutorial') {
            pathstr =
              '../../docs/' + file.path.replace(/\/index.html/, '.zh.md');
          }
          ...
          if (
            ...
            pathstr =
              '../../' +
              pathItems.slice(1).join('/').replace(/html/, `${prefix}.md`);
          }
          if (pathItems[0] === 'examples') {
            pathstr =
              '../../' + pathItems.join('/').replace(/\.html/, '.zh.md');
          }
          ...
          if (pathItems[0] === 'examples' || pathItems[1] === 'examples') {
            const metaPath =
              md.split('/').slice(0, -1).join('/') + '/demo/meta.json';
